...
open class TabView: UIScrollView {
    ...
    fileprivate func setupContainerView(dataSource: TabViewDataSource) {
        ...
        case .flexible:
            ...
            if #available(iOS 11.0, *), options.isSafeAreaEnabled {
                ...
                containerView.frame = CGRect(x: 0, y: options.margin + safeAreaInsets.left+200, width: containerWidth, height: containerHeight)
            } else {
    ...
    fileprivate func setupTabItemViews(dataSource: TabViewDataSource) {
        ...
        for index in 0..<itemCount {
            let tabItemView = TabItemView(frame: CGRect(x: xPosition, y: 100, width: options.itemView.width, height: containerView.frame.size.height))
            ...
            case .flexible:
                if options.needsAdjustItemViewWidth {
                    ...
                    adjustCellSize.width = tabItemView.titleLabel.sizeThatFits(containerView.frame.size).width + options.itemView.margin * 20
                    tabItemView.frame.size = adjustCellSize
                    tabItemView.frame.size.width = screenWidth/2 - options.itemView.margin * 2
