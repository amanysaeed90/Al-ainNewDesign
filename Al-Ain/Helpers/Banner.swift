...
open class Banner: UIView {
    ...
    open var position = BannerPosition.bottom
    ...
    open let detailLabel: UILabel = {
        let label = UILabel()
        label.font = UIFont(name: "DroidArabicKufi", size: 13)!
        label.numberOfLines = 0
    ...
    @objc internal func didTap(_ recognizer: UITapGestureRecognizer) {
        if dismissesOnTap {
            dismiss()
                   RootViewController.shared?.navigationController?.popToRootViewController(animated:  true)
                   RootViewController.shared?.tabBarController?.selectedIndex = 0
